-- Nicole (client) tested registration and did not see Full Name / Email
-- Address as part of the intake experience, since they're already
-- collected at signup and the intake form only asked for what signup
-- didn't have. Her spec lists Full Name and Email Address as intake
-- Questions 1 and 2, so they need to visibly appear as part of intake,
-- not be silently satisfied elsewhere. Email stays read-only (it's the
-- auth identifier -- changing it here would mean changing the Supabase
-- Auth account email, out of scope and risky); Full Name (first/last)
-- becomes a real, editable part of this RPC.

-- Adding parameters changes the function's signature -- create or replace
-- would leave the old 13-param overload in place alongside this new
-- 15-param one (same trap as 20260902000004_drop_stale_ensure_participant_overload.sql).
-- Drop the old signatures explicitly first.
drop function if exists public.save_participant_intake(
  text, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean, text, boolean
);
drop function if exists public.admin_update_participant_intake(
  uuid, text, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean, text, boolean
);
...
-- Mirror the same first/last name editability on the admin edit path.
